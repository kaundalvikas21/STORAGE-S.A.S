...
export default function TrustBar() {
  return (
    <section aria-labelledby="trust-title" className="order-8 bg-primary text-on-primary">
        ...
        <Reveal group as="ul" className="grid grid-cols-2 md:grid-cols-5 gap-x-6 gap-y-10" role="list">
          {trust.map((t) => (
            <RevealItem as="li" key={t.label} className="flex flex-col items-start md:items-center md:text-center">
              <AnimatedNumber value={t.value} suffix={t.suffix} className="font-display text-3xl md:text-[2.75rem] font-semibold leading-none text-accent" />
              <span className="mt-3 font-display text-[15px] md:text-base text-on-primary/85">{t.label}</span>
            </RevealItem>
          ))}
        </Reveal>
        <p className="mt-10 text-[14px] text-on-primary/70 max-w-[70ch] md:mx-auto md:text-center">
