...
Marketing site for Storage S.A.S (minibodegas, Bogotá), Spanish (`es-CO`). Next.js 16 App Router (`searchParams` is a Promise), React 19, Tailwind 3, framer-motion, Phosphor icons. Routes: `/` (`app/(site)/page.tsx`), `/bodegaje-bogota/` silo pillar (`app/(site)/bodegaje-bogota/page.tsx`, blocks `AxisChooser`/`IncludedGrid`/`PriceBand`/`FaqAccordion` in `components/bodegaje/` plus the shared `components/PageIntro.tsx` and the homepage `SegmentStrip` reused with `title`/`body`/`more` props, copy in `bodegajePillar`; size cards point at `/cotizar/` until those pages ship, marked `SWAP`), the five segment pages (spec T3: `/minibodegas-para-hogar/`, `/minibodegas-para-empresas/`, `/bodegas-para-ecommerce/`, `/almacenamiento-de-archivo-y-documentos/`, `/bodegas-para-constructoras/`; each `page.tsx` only exports `segmentMeta(slug)` and renders `components/segmento/SegmentPage.tsx`, blocks `UseCases`/`M3Guide`/`SizeRecommender`/`SecurityTeaser`/`BlogTeasers` in `components/segmento/` plus `AxisChooser variant="sede"`, copy in `segmentPages` in segments.ts, Service + FAQPage JSON-LD; `BlogTeasers` reads title + href from content/articles.ts), the four size pages (spec T8, silo 1B: `/bodegas-pequenas/`, `/bodegas-medianas/`, `/bodegas-grandes/`, `/espacios-personalizados/`; each `page.tsx` only exports `sizeMeta(id)` and renders `components/tamanos/SizePage.tsx`, keyed by `sizes[].id`; `robots: noindex` until spec Open Item 2 (occupancy file) closes, UNBLOCK CHECKLIST in SizePage.tsx; blocks `PageIntro` (range aside) → `M3Guide` → `PriceBand` → `AvailabilityBySede` (reads `sedeDetails[slug].sizesAvailable`, 197 first) → `SizeCompareStrip` → `ClosingBand`, copy in `sizePages`/`sizePage` in content/tamanos.ts, Service JSON-LD until a price allows Product/Offer), the blog (spec T9, silo 5): `/blog/` hub (`app/(site)/blog/page.tsx`: `PageIntro actions={false}`, client `components/blog/BlogIndex` with category chips mirrored in `?categoria=`, fed plain props so post bodies never reach the browser, `ClosingBand`, Blog JSON-LD from `lib/schema-blog.ts`) and six posts `/blog/{slug}/` (`app/(site)/blog/[slug]/page.tsx`, static params, unknown slugs 404; `ArticleHeader` (container-width banner)/`ArticleBody` (+ `ArticleToc` from the h2s)/`InlineCta`/`NextStepBand` in `components/blog/`, Article JSON-LD; slugs are ours, PENDIENTE CONFIRMAR; every post ends in `NextStepBand` linking UP into silo 1, never sideways out of the blog), `/mudanzas-bogota/` pillar (spec T2b, `app/(site)/mudanzas-bogota/page.tsx`, the bodegaje pillar kit with props: `AxisChooser variant="services"` (children → `/cotizar/`, `SWAP`), `IncludedGrid items=`, `HowItWorks steps=`, `PriceBand link={null}`, plus `components/mudanzas/BridgeToBodegaje`, the ONE cross-silo body link, mudanzas → bodegaje only: nothing in the bodegaje silo links back; copy in `mudanzasPillar`, content/mudanzas.ts), `/sedes/` hub (`app/(site)/sedes/page.tsx`, blocks in `components/sedes/`, copy in `sedesHub`), the four sede pages `/sedes/{slug}/` (`app/(site)/sedes/[slug]/page.tsx`, static params from `sedePages`, unknown slugs 404; per-page data in `sedeDetails`, shared labels in `sedePage`, blocks `SedeHero`/`SedeGallery`/`SizesAtSede`/`AccessSecurity`/`SedeLocation`/`ZonesServed`/`SedeReviews` in `components/sedes/`, one `selfStorage()` JSON-LD per physical point), the company pages (spec T6, all `PageIntro` + blocks + FAQ + `ClosingBand`): `/precios/` (`components/precios/` `PriceTable`/`PriceFactors`/`IncludedVsExtra`, copy in `preciosPage`, content/pricing.ts; each row's optional `priceRange` replaces the "Cotiza tu tarifa" link once Open Item 4 closes), `/seguridad/` (`components/seguridad/SecurityFeatures`, `seguridadPage`), `/preguntas-frecuentes/` (`components/faq/FaqHub`, `faqHub.groups` in faqs.ts, one FAQPage) and `/contacto/` (QuoteForm + `components/contacto/ContactRail` + `SedeLocation points={sedes}`, ContactPage + 7 SelfStorage JSON-LD, `contactoPage`), `/cotizar/` and `/calculadora-de-espacio/` (`app/(conversion)/`), `POST /api/lead` (`app/api/lead/route.ts`). `trailingSlash: true`. Brief and wireframe: `Storage_SAS_Wireframe_Silo_Spec.xlsx` / `Storage_SAS_Wireframe_Visual.html`; inner pages use the spec's URLs only, never invented slugs. The site may be rebuilt in WordPress after approval: content stays in plain typed objects, sections stay self-contained blocks, calculator/lead logic stays framework-free, no server actions.

## Architecture (the parts that span files)

**Content is split by domain, imported through `content/site.ts`.** `facts.ts` (URLs, `company`/NAP, size bands `sizes` with `id`/`label`/`maxM3`, `trust` stats, `securityLine`), `sedes.ts`, `segments.ts`, `faqs.ts` (`faq`, `calcFaq`, `preciosFaq`, `seguridadFaq`, `permanenciaFaq`, `faqHub`; no price figures anywhere until Open Item 4 closes, removed from the homepage 2026-09-15), `conversion.ts` (page copy for /cotizar/ and the calculator), `engagement.ts`, `bodegaje.ts` (pillar copy), `mudanzas.ts` (mudanzas pillar copy), `pricing.ts`, `seguridad.ts`, `contacto.ts`, `articles.ts` (blog index: categories, `blogHub`, `articles` meta; bodies are `Block[]` in `content/blog/{slug}.ts` with inline links written `[label](/ruta/)`, gathered in `content/blog/index.ts`, imported by pages only; `lib/articles.ts` `splitLinks`/`readingMinutes`/`headingId` are zero-import and checked by scripts/check-conversion.mjs); `site.ts` keeps nav/footer/silos/steps/reviews/clients and re-exports the rest. Components import from it (client components may import the domain file directly to keep bundles small); no prose or contact data hardcoded in JSX. Values tagged `PENDIENTE CONFIRMAR` are awaiting the client: swap them there only. Never invent prices, dimensions, reviewer names or phone numbers.
...
**Server/client split:** sections are server components by default. Only `Header` (+ `header/MegaMenu`, `header/MobileDrawer`), `Reveal`, `AnimatedNumber`, `IntentCards`, `ShowcaseCycler`, `ScrollRow`, `Magnetic`, `SedeMap` + `SedeLeafletMap` (Leaflet on every breakpoint, lazy chunk loaded as the map nears the viewport; touch devices scroll the page with one finger and pinch to zoom), `SedeList` (lays out the sedes header tools, map and cards it receives server-rendered as props; owns the nearest-first sort) `SocialGrid` (post filter; `SocialTile` itself is a server component), `Popups` (newsletter + exit-intent `<dialog>`, mounted in `app/(site)/layout.tsx`), `WhatsAppWidget` + `BackToTop` (mounted in `app/layout.tsx`) `blog/BlogIndex` (category filter, fed plain props) + `blog/ArticleToc` (current-section tracking via IntersectionObserver) and `conversion/QuoteForm` + `conversion/Calculator` are `"use client"`. `Photo` (components/Photo.tsx, renders content/images.ts slots) is a server component. Keep that boundary — importing a client-only hook into a section without the directive is the build error that bit last time.
...
`public/img/` holds the hero background plates (`hero_img_bg*.png`, 1672x941, shot with an empty left band for the headline); the hero uses `hero_bg.png` (yellow-door facility, forklift, Monserrate; the earlier `hero_img_bg_1..3` plates are unused). Every other photo slot (Bodegaje showcase `showcase-1..3`, Mudanzas `silo-mudanzas`, segments `segment-hogar`/`segment-empresa`, closing `cta-closing`, 7 sede thumbnails `sede-*`) is a generated local image mapped in `content/images.ts`; `sede-alt-3` is the /sedes/ coverage photo (`coveragePhoto`); sede page galleries are `sedeGallery` (keyed by slug, 4 each, no photo repeated across pages: the sede facades, `sede-alt-2` on Autopista Norte, `segment-empresa-alt` on Paloquemao, plus the 1536x1024 `sede-autopista-norte-acceso/-pasillo`, `sede-toberin-pasillo`, `sede-spring-entrada/-pasillo/-bodega`, `sede-paloquemao-carga`); `zonePhotos` (keyed by slug) is the catchment photo beside each sede page's ZonesServed prose (`zona-sabana-norte`, `zona-usaquen-cedritos`, `zona-suba-colina`, `zona-centro-paloquemao`; a slug without one keeps the band in one column); `sede-alt-1` is the /bodegaje-bogota/ intro photo (`pillarPhoto`); `seguridad-candado` (`seguridadPhoto`, /seguridad/ intro), `seguridad-cctv` (`cctvPhoto`, the CCTV bento cell) and `precios-tamanos` (`preciosPhoto`, /precios/ intro) are the company-page plates. Segment intros use `segmentPhotos` (keyed by slug: `segment-hogar`, `segment-empresa`, `segment-ecommerce`, `segment-archivo`, `segment-obra`; all 1536x1024). /mudanzas-bogota/ uses `silo-mudanzas` (intro), `trasteoPhoto` (reuses `zona-usaquen-cedritos`) and `showcase-2` (bridge). No external image host remains (the Unsplash `remotePatterns` entry was removed from `next.config.mjs`). Social posts live in `public/social/`.
